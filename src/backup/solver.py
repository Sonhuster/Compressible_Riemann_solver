...
def cal_left_right_state(mesh, fw, var):
    ...
    state_L = np.vstack((rhoL, uL, vL, pL, hL, eL, TL))
    state_R = np.vstack((rhoR, uR, vR, pR, hR, eR, TR))
...
def AUSM_flux(mesh, state_L, state_R):
    ...
    rhoL, uL, vL, pL, hL, eL, TL = state_L[0], state_L[1], state_L[2], state_L[3], state_L[4], state_L[5], state_L[6]
    rhoR, uR, vR, pR, hR, eR, TR = state_R[0], state_R[1], state_R[2], state_R[3], state_R[4], state_R[5], state_R[6]
    ...
    (_sn, ) = mesh.var_elem_wise(mesh.global_faces.sn, )
    outward_sn = np.einsum('nij,ni->nij', _sn, snsign)

    MaL = np.einsum('ijk,ijk->ij', np.dstack((uL, vL)), outward_sn) / cL
    MaR = np.einsum('ijk,ijk->ij', np.dstack((uR, vR)), outward_sn) / cR
    ...
    rho_flux  += 0
    rhou_flux += outward_sn[:, :, 0] * (pp + pm)
    rhov_flux += outward_sn[:, :, 1] * (pp + pm)
    rhoe_flux += 0
...
def ROE_flux(mesh, state_L: list, state_R: list):
    ...
    uL = state_L[1]
    vL = state_L[2]
    unL = uL*outward_sn[:, :, 0] + vL*outward_sn[:, :, 1]
    ...
    FL = np.array([
        rL * unL,
        phiL[1] * unL + pL * outward_sn[:, :, 0],
        phiL[2] * unL + pL * outward_sn[:, :, 1],
        rHL * unL
    ])
    ...
    uR = state_R[1] / rR
    vR = state_R[2] / rR
    unR = uR * outward_sn[:, :, 0] + vR * outward_sn[:, :, 1]
    ...
    rHR = phiR[3] + pR
    HR = rHR / rR
    cR = np.sqrt(shr * pR / rR)

    # right flux
    FR = np.array([
        rR * unR,
        phiR[1] * unR + pR * outward_sn[:, :, 0],
        phiR[2] * unR + pR * outward_sn[:, :, 1],
        rHR * unR
    ])
    ...
    af = 0.5 * (ui * ui + vi * vi)
    ucp = ui * outward_sn[:, :, 0] + vi * outward_sn[:, :, 1]
    c2 = (shr - 1) * (Hi - af)
    if np.any(c2 < 0):
        raise ValueError('Non-physical state!')
    ci = np.sqrt(c2)
    ...
    G2 = -ucp * du[0] + du[1] * outward_sn[:, :, 0] + du[2] * outward_sn[:, :, 1]
    ...
    F = [
        ...
        0.5 * (FL[1] + FR[1]) - 0.5 * (l3 * du[1] + C1 * ui + C2 * outward_sn[:, :, 0]),
        0.5 * (FL[2] + FR[2]) - 0.5 * (l3 * du[2] + C1 * vi + C2 * outward_sn[:, :, 1]),
...
def diffusion_flux(mesh, var, state_L, state_R):
    ...
    u_grad_f = (uR - uL) / _delta
    v_grad_f = (vR - vL) / _delta
    T_grad_f = (TR - TL) / _delta

    # nx, ny = _sn[:, :, 0] * snsign, _sn[:, :, 1] * snsign
    abs_nx, abs_ny = np.abs(_sn[:, :, 0] * snsign), np.abs(_sn[:, :, 1] * snsign)

    u_grad_fx, u_grad_fy = u_grad_f * abs_nx, u_grad_f * abs_ny
    v_grad_fx, v_grad_fy = v_grad_f * abs_nx, v_grad_f * abs_ny
    T_grad_fx, T_grad_fy = T_grad_f * abs_nx, T_grad_f * abs_ny

    div_u = u_grad_fx + v_grad_fy
    div_v = u_grad_fy + v_grad_fy

    tau_xx = mu * (2 * u_grad_fx - 2 / 3 * div_u)
    tau_yy = mu * (2 * v_grad_fy - 2 / 3 * div_v)
    tau_xy = tau_yx = mu * (u_grad_fy + v_grad_fx)

    rho_fx  = np.zeros((mesh.no_elems(), mesh.no_local_faces()))
    rhou_fx = tau_xx
    rhov_fx = tau_xy
    rhoe_fx = tau_xx * _uf + tau_xy * _vf + k_Th * T_grad_fx

    rho_fy  = np.zeros((mesh.no_elems(), mesh.no_local_faces()))
    rhou_fy = tau_yx
    rhov_fy = tau_yy
    rhoe_fy = tau_yx * _uf + tau_yy * _vf + k_Th * T_grad_fy

    mask_in_neumann = ~ bc.get_outflow_bc_mask(mesh, SIMULATION_MODE)
    (_mask_in_neumann, ) = mesh.var_elem_wise(mask_in_neumann, )
    rho_flux    = (abs_nx * rho_fx + abs_ny * rho_fy)  * _mask_in_neumann
    rhou_flux   = (abs_nx * rhou_fx + abs_ny * rhou_fy) * _mask_in_neumann
    rhov_flux   = (abs_nx * rhov_fx + abs_ny * rhov_fy) * _mask_in_neumann
    rhoe_flux   = (abs_nx * rhoe_fx + abs_ny * rhoe_fy) * _mask_in_neumann

    diff_flux = [rho_flux, rhou_flux, rhov_flux, rhoe_flux]

    # check = np.round(np.sum(rhou_flux * _area, axis=1), 2)
    return diff_flux
...
def intergrate_flux(mesh, conv_flux, diff_flux):
    ...
    rho_flux,  = ufs.get_total_flux(_area * (conv_flux[0] - diff_flux[0]), ) / vol
    rhou_flux, = ufs.get_total_flux(_area * (conv_flux[1] - diff_flux[1]), ) / vol
    rhov_flux, = ufs.get_total_flux(_area * (conv_flux[2] - diff_flux[2]), ) / vol
    rhoe_flux, = ufs.get_total_flux(_area * (conv_flux[3] - diff_flux[3]), ) / vol
