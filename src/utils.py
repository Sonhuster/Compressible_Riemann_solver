...
def check_symmetry_matrix_structured(arr, iter=None, rk_idx=None):
    if len(arr.shape) == 2:
        arr = np.sum(arr, axis=1)

    arr_symm1 = np.abs(arr[0:3])
    arr_symm2 = np.abs(arr[-3:][::-1])

    if iter is not None and rk_idx is not None:
        assert np.all(np.isclose(arr_symm1, arr_symm2)), f"Non-symmetric flow at {iter}, {rk_idx}"
    else:
        assert np.all(np.isclose(arr_symm1, arr_symm2)), f"Non-symmetric flow"
